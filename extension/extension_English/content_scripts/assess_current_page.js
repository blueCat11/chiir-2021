...
function populateNudgeModal(nudge, nudge_explanation, study, date, isSmallBad, nudge_value, betterHeader1, betterHeader2){
  ...
  let studyEndDate = new Date(2020, 5, 15);
  studyEndDate = studyEndDate.toLocaleDateString('de-DE', dateOptions);
  let dateDate = new Date(2020, 5, 2);
  dateDate = dateDate.toLocaleDateString('de-DE', dateOptions)
  studyDescription.innerHTML = study.header.concat(" from ", studyStartDate, " to ", studyEndDate);
  ...
  if (study.avg_total != null && study.min_total != null && study.max_total != null){
    ...
    if(study.own_value == null){
      ...
      betterThanStudy.innerHTML = "There is no data available for you in this time period.";
    }
  }
  ...
  if (date.avg_total != null && date.min_total != null && date.max_total != null){
    ...
    if(true){
      ...
      betterThanDate.innerHTML = "There is no data available for you in this time period.";
    }
  }

  let study_own_value = (study.avg_total - study.min_total)/2
  let date_own_value = (date.avg_total - date.min_total)/2
  drawVisualizationForNudge("alertCanvas_study", possWidthForCanvas, study.min_total, study.max_total+8, study.avg_total, study_own_value, isSmallBad);
  drawVisualizationForNudge("alertCanvas_date", possWidthForCanvas, date.min_total, date.max_total+6, date.avg_total+1, date_own_value, isSmallBad);
  let closeButton = document.getElementById("closeModal");
  closeButton.addEventListener("click", function(){
...
function drawVisualizationForNudge(canvasId, canvasWidth, min, max, avg, you, isSmallBad){
  ...
  if (avg == null || min == null || max == null){
    ...
    con.fillText("No data available for this time period", canvasWidth/2, canvasHeight/2 - textHeight);
  }else{
    ...
    if (youLocationX != undefined){
      ...
      con.stroke();
      con.fillText("You", youLocationX, barUpperY-textAbove)
    }

    con.moveTo(avgLocationX, barUpperY);
    con.lineTo(avgLocationX, barUpperY+barHeight);
    con.stroke();
    con.fillText("Average", avgLocationX, barUpperY+barHeight+textHeight)
