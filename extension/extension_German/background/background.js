...
let previousInterventionDate = new Date(2020, 4, 1); //the date when the last intervention was shown, starts with before the study, but is updated to check whether a new information can be displayed to participantIdForExtension
//current data are 2020-May-1 (months start with 0)
...
// TODO change with own data (for all dates)
let interventionStartDate = new Date(2020, 5, 1); // this is when the people in boost- and nudge-groups will start seeing extra information
let studyStartDate = new Date(2020, 4, 25) // current data 2020-May-25
let interventionEndDate = new Date(2020, 5, 8) // this is when the people in boost and nudge groups don't get shown any more info
let studyEndDate = new Date(2020, 5, 15) // on this date, the participants are prompted to fill out questionnaires and are then shown all data
...
let study_header = "Während der gesamten Studiendauer";
let date_header = "Am letzten vollständigen Tag: ";
let better_header_1 = "Damit ist Ihr Browsingverhalten privater als das von ";
let better_header_2 = "% der Teilnehmer";
...
let totalBoosts = [
  "Durch das Nutzen eines Adblockers werden, bereits ohne die Blocker-Einstellungen zu ändern, die Anzahl der Drittanbieter auf einer Seite um 40% reduziert.",
  "Webseiten, die sich mit Unterhaltung beschäftigen haben mehr Cookies und Drittanbieter pro Seite, als die meisten anderen Arten von Webseiten.",
  "Webseiten, die sich mit (Weiter-)Bildung beschäftigen haben weniger Cookies und Drittanbieter pro Seite, als die meisten anderen Arten von Webseiten.",
  "Durch das Nutzen des privaten Modus (Firefox) bzw. Inkognito Modus (Chrome) werden Cookies nach dem Schließen des Browsers automatisch gelöscht.",
  "Das Blockieren von Drittanbieter-Cookies (Third Party Cookies) in den Browsereinstellungen führt zu einer Verringerung der Anzahl Drittanbieter pro Seite von circa 30%."
...
let totalNudges = [
  ...
  explanation: "Durchschnittliche Gesamtanzahl von erreichten Privatsphäre-Punkten pro besuchter Seite",
  isSmallBad: true
  },
  {value: "PrivacyPoints_SettingChange",
  explanation: "Anzahl der durch Browsereinstellungen erreichten Privatsphäre-Punkte (Durchschnitt pro Seite)",
  isSmallBad: true
  },
  {value: "PrivacyPoints_WebsiteVisits",
  explanation: "Anzahl der durch Webseitenbesuche erreichten Privatsphäre-Punkte (Durchschnitt pro besuchter Seite)",
  isSmallBad: true
  },
  {value: "Num3rdPartyRequests",
  explanation: "Durchschnittliche Gesamtanzahl von Kontaktaufnahmen zu Dritt-Parteien (Third Party Requests) pro besuchter Seite",
  isSmallBad: false
  },
  {value: "NumCookies",
  explanation: "Durchschnittliche Gesamtanzahl von Cookies pro besuchter Seite",
  isSmallBad: false
  }
  ...
  async function sendThirdParty(response) {
    if (response.tabId != -1){
      browser.tabs.get(response.tabId, function(tab){
        ...
        let originUrl = getOriginUrl(response);
...
function displayIntervention(timeStamp){
  ...
  if ((timeStamp >= interventionStartDate) && (timeStamp < interventionEndDate) && (previousInterventionDate.getDate() != timeStamp.getDate()) ) {
    ...
    previousInterventionDate = timeStamp; //comment this for testing
    //console.log(conditionForExtension);
...
/*    window.browser.storage.local.set({
    ...
    */
    ...
    // api call looks like: http://127.0.0.1:8000/api/website_categorizations/?domains=p,a,jjj
      ...
      .then(websiteCategorizations => {
        ...
        for(let m = 0; m < websiteCategorizations.length; m++){
          ...
          if(index != -1){
            ...
            console.log("ATTENTION, current website not in localWebsitesCategoriation: ", domain)
          }
